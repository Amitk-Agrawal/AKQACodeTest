...
namespace AKQA.CodeTest.Service
{
    public class AmountHandler
    {
        ...
        private static string NumberToWords(decimal amount)
        {
            ...
            if (remaining > 0)
            {
                ...
                if (remaining < 20)
                {
                    strAmount.Append(arrNumber[Convert.ToInt16(remaining)]);
                }
                else
                {
                    strAmount.Append(arrNumberTens[(int)remaining / 10]);
                    if (((int)remaining / 10) > 0)
                        strAmount.AppendFormat(" {0}", arrNumber[(int)remaining % 10]);
                }
            }
        ...
        {
            return Convert.ToInt16(Math.Abs((amount - Math.Truncate(amount)) * 100));
        }
    }
}